...
The methodology this is running on is [Domain Foundation](/work/domain-foundation/) — the encoded-institutional-expertise approach I developed out of a year of research at Oracle. Same mechanism as the org version, different scale. Context encoded in a form a model can reach becomes the thing that lets any size of operation reflect its actual practice instead of the industry average.

Commitment lives in OmniFocus. Eight projects — three for the active job search, three for life's running categories (household, the ongoing backlog, short-term priorities), one catch-all inbox, and one I haven't renamed yet. Seven tags, used in exact case. A skill called `omnifocus-expert` holds all of that, plus the rule that a flagged task is a commitment for the day rather than a wish for the future. When I capture a task in conversation with Claude, the skill routes it — inbox if context is unclear, the right project if it isn't, a defer date if it's genuinely time-gated. The system stays accurate because the capture step is cheap.

Obsidian holds thinking. There are four zones, each with its own conventions: one inherits the research vocabulary from my last job, one holds hyperfocus project work, one is for songwriting, one is for AI workflow assets. Two skills govern it — `obsidian-pkm` describes the vault structure, `obsidian-cli` covers the command-line interface for scripted operations. Together they're the difference between a note system and a knowledge system. Any skill that needs to park a thought can park it in the right zone with the right frontmatter without me thinking about it.

Procedural knowledge lives in skills. Twenty-six of them, each a `SKILL.md` and for a few, a `references/` subfolder for corpus material. I [wrote about the ecosystem](/work/skill-ecosystem/) at length. The relevant point here is that skills don't just govern work — they hold context the other subsystems reach for. `omnifocus-expert` knows which projects are time-gated and when. `obsidian-pkm` knows the frontmatter and tag schema the research zone requires. The rulebooks for how other systems behave live here.
...
Then there's the physical world. A 2013 Mac Pro in my basement runs Ubuntu Server and Home Assistant Container (not Supervised, which means no add-on store and manual orchestration for everything). The `home-assistant-expert` skill knows that specific stack. Lights, climate, a local voice pipeline slowly being built out, Z-Wave and Zigbee devices throughout the house. The OS extends into the physical world because the physical world is where the work actually happens — walking to a mesh radio node to check its uplink isn't a productivity concern, it's a location in the day.
